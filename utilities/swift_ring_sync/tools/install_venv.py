...
"""
Installation script for swift-ring-sync's development virtualenv
"""
...
VENV = os.path.join(ROOT, '.swift-ring-sync-venv')
...
def install_dependencies(venv=VENV):
    ...
    # Tell the virtual env how to "import swift-ring-sync"

    for version in ['python2.7', 'python2.6']:
        pth = os.path.join(venv, "lib", version, "site-packages")
        if os.path.exists(pth):
            pthfile = os.path.join(pth, "swift-ring-sync.pth")
            f = open(pthfile, 'w')
            f.write("%s\n" % ROOT)
...
def print_help():
 ...
 $ source .swift-ring-sync-venv/bin/activate
